<template>
  <div class="h-screen flex flex-col bg-gray-900 text-white">
    <!-- Topbar -->
    <header
      class="h-16 bg-gray-800 flex items-center px-4 shadow justify-between"
    >
      <div class="flex items-center space-x-4">
        <div class="text-lg font-bold">Círius CMS</div>
        <!-- Botão de controle da sidebar -->
        <button @click="toggleSidebar" class="bg-gray-700 px-4 py-2 rounded">
          <span class="block w-5 h-0.5 bg-gray-800"></span>
          <span class="block w-5 h-0.5 bg-gray-800 mt-1"></span>
          <span class="block w-5 h-0.5 bg-gray-800 mt-1"></span>
          <!-- {{ sidebarOpen ? "Fechar Menu" : "Abrir Menu" }} -->
        </button>
      </div>
      <div class="flex flex-wrap space-x-4">
        <!-- <button @click="refreshTree++" class="bg-blue-500 px-4 py-2 rounded">
          Refresh
        </button> -->
        <!-- <button
          @click="tabSet('metatags')"
          class="bg-gray-700 px-4 py-2 rounded"
        >
          Metatags
        </button> -->
        <!-- <button
          @click="tabSet('content')"
          class="bg-gray-700 px-4 py-2 rounded"
        >
          Conteúdo
        </button> -->
        <button
          @click="compiler.compileSite"
          :disabled="compiler.loading"
          class="bg-green-600 px-4 py-2 rounded"
        >
          {{ compiler.loading ? "Publicando..." : "Publicar Site" }}
        </button>
      </div>
    </header>
    ...
    <div class="flex-1 flex">
      ...
      <aside
        :class="[
          'bg-gray-800 flex text-white transition-all duration-300 ease-in-out border-r border-gray-700',
      ...
      <!-- Editor Area -->
      <main
        :class="['flex-1 flex-col h-screen', sidebarOpen ? 'w-full md:w-2/3' : 'w-full']"
        class="transition-all duration-300 "
      >
        <!-- Barra de Título -->
        <div
          class="bg-gray-700 text-white py-1 px-4 flex items-center justify-between"
        >
          <div>
            <span class="font-bold text-sm">Edição: </span><span>{{ nodeSelected._path }}</span>
          </div>
          <button type="button" @click="submitForm" class="bg-blue-500 px-4 py-2 rounded">
            Salvar
          </button>
        </div>
        <form @submit.prevent="submitForm" class=" p-4">
          ...
          <div class="mb-4">
            ...
            <input
              ...
              type="text"
              id="label"
              class="mt-1 block w-full bg-gray-800 text-white p-2 border-gray-600 rounded"
              required
            />
          </div>
          ...
          <!-- Slug Input -->
          <div class="mb-4">
            <label for="slug" class="block text-sm font-medium text-gray-200"
              >Slug</label
            >
            <input
        ...
        <!-- Barra de Título -->
        <div
          class="bg-gray-700 text-white py-2 px-4 flex items-center justify-between"
        >
          <div>
            <span class="font-bold text-sm">Conteúdo</span>
          </div>
        </div>
        <!-- Markdown Editor -->
        <div class="_flex-1_ _h-[487px]">
          <!-- <label
              for="content"
          ...
          <textarea
            ...
            id="content"
            class="w-full h-[300px] bg-gray-800 text-white  text-[24px] resize-none p-2 rounded focus:outline-none"
          ></textarea>
        </div>
      </main>
      <!-- Preview Column -->
      <aside class="w-1/3 bg-gray-600 flex overflow-auto  hidden md:block">
        <!-- Barra de Título -->
        <div class="bg-gray-700 text-white py-2 px-4">
    ...
    <!-- Footer -->
    <footer class="h-8 bg-gray-500 flex items-center justify-center text-sm">
      <span>&copy; 2024 Círius CMS - Todos os direitos reservados</span>
    </footer>
  </div>
</template>
